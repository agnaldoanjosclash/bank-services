...
public class AccountControllerTest {
    ...
    void whenBalance_thenReturnsAccountBalance() throws Exception {
        ...
        mockMvc.perform(get("/kernel/account/{number}/agency/{agency}/clientDocument/{clientDocument}/typeDocument/{typeDocument}/balance",
                        "123", "001", "12345678901", "CPF"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void whenTransfer_thenReturnsMessageResponse() throws Exception {
        ...
        when(cashOperationsService.transfer(any(TransferRequestDTO.class))).thenReturn(localMessageResponseDTO);

        mockMvc.perform(post("/kernel/account/transfer")
    ...
    @Test
    void whenTransaction_thenReturnsMessageResponse() throws Exception {
        var localMessageResponseDTO = MessageResponseDTO.builder()
                .id(UUID.randomUUID().toString())
                .description(UUID.randomUUID().toString())
                .status(MessageStatus.SUCCESS)
                .data(TransferResponseDTO.builder().transactionId(UUID.randomUUID().toString()).build())
                .build();
        when(cashOperationsService.transactionQuery(any(String.class))).thenReturn(localMessageResponseDTO);

        mockMvc.perform(get("/kernel/account/transaction/{transactionId}/details", "txn123"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.status").value(localMessageResponseDTO.getStatus().toString()));
    }
    ...
    @Test
    void whenTransactionNotFound_thenReturnsNotFound() throws Exception {
        when(cashOperationsService.transactionQuery(any(String.class))).thenReturn(null);

        mockMvc.perform(get("/kernel/account/transaction/{transactionId}/details", "txnNotFound"))
                .andExpect(status().isNotFound());
    }
}
